...
function runWLCDH(cfg, K = 2) {
  ...
  for (const block of blocks) {
    ...
    // Discover incoming dataflow edges for this block by inspecting all uses
    block.incomingDataflowEdges = [];
    
    function extractDataflow(node, parentRole = 'unknown') {
      if (!node) return;
      if (Array.isArray(node)) { node.forEach((n, i) => extractDataflow(n, `${parentRole}[${i}]`)); return; }
      if (typeof node === 'object') {
        if (node.reachingDefinitions) {
          for (const def of node.reachingDefinitions) {
            if (def._df_block) block.incomingDataflowEdges.push({ block: def._df_block, role: parentRole });
          }
        }
        for (const key in node) {
          if (key === 'cfgEntry' || key.startsWith('_')) continue;
          extractDataflow(node[key], key);
        }
      }
    }
    extractDataflow(block.instructions, 'root');
  }

  // Iterative Contextual Aggregation
  for (let k = 1; k <= K; k++) {
    for (const block of blocks) {
      const Din = block.incomingDataflowEdges
        .map(e => `${e.role}:${e.block.signatures[k-1]}`)
        .sort();
      
      const Cin = block.predecessors
        .map(b => `CFG:${b.signatures[k-1]}`)
        .sort();
      
      // Strict domain separation using delimiters
